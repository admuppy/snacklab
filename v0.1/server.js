...
// 확장자 없는 콘텐츠 원문(guide.md·explain.md)의 언어. 저작 규약상 항상 한국어이며 DEFAULT_LOCALE
// (포털 UI 기본값 — 배포마다 en 등으로 바뀐다)과는 별개다. 둘을 묶어 두면 DEFAULT_LOCALE=en 배포에서
// 한국어 사용자에게 guide.en.md 가 나간다.
const SOURCE_LOCALE = LOCALES.includes(process.env.CONTENT_SOURCE_LOCALE) ? process.env.CONTENT_SOURCE_LOCALE : 'ko';
...
// 단 요청이 원문 로케일(SOURCE_LOCALE, 기본 ko)이면 확장자 없는 원문(<base>.md)이 그 로케일의 정본이므로
// 먼저 본다 — 이 순서를 빠뜨리거나 DEFAULT_LOCALE 로 판정하면 한국어 사용자에게 영어 가이드가 나간다.
const contentFiles = (base, lang) => [
  `${base}.${lang}.md`,
  ...(lang === SOURCE_LOCALE ? [`${base}.md`] : []),
...
function hasNativeContent(labId, lang) {
  return fs.existsSync(labPath(labId, `guide.${lang}.md`))
    || (lang === SOURCE_LOCALE && fs.existsSync(labPath(labId, 'guide.md')));
}
