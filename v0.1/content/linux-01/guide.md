...
- `id` — 현재 사용자의 UID, 1차 그룹(`gid`), 소속된 모든 그룹(`groups`)을 보여 준다.
...
- `umask` — 새 파일/디렉터리에서 **빼 버릴** 권한 비트. 흔한 `0022` 는 그룹·기타의 쓰기(w)를 뺀다 → 파일 644, 디렉터리 755.
...
- `touch` — 빈 파일을 만든다(있으면 수정 시각만 갱신). `&&` — 앞 명령이 성공하면 이어서 실행.
- `stat -c %a` — 형식 지정(`-c`)으로 권한을 8진수(`%a`)만 출력한다.
...
- `mkdir -p` — 상위 경로까지 만들고 이미 있어도 에러 없음.
- `chmod 700` — 8진수로 권한 지정: 소유자 rwx(7), 그룹·기타 없음(0).
...
- `echo "…" > 파일` — 문자열을 파일에 쓴다(없으면 생성, 있으면 덮어쓰기). 새 파일 권한은 umask 를 따른다.
...
- `600` — 소유자 rw(6 = 4+2), 그룹·기타 없음. 심볼릭으로는 `chmod u=rw,go= <파일>`.
...
- `%a` 8진수 권한, `%n` 파일 이름. 파일을 여러 개 주면 한 줄씩 출력한다.
...
- `groupadd` — 그룹 생성. `-f` 는 이미 있어도 에러 없이 성공으로 끝낸다.
...
- `chgrp <그룹> <경로>` — 소유 그룹만 바꾼다. 소유자까지 바꾸려면 `chown 사용자:그룹`.
...
- 네 자리 8진수의 맨 앞 자리가 특수 비트: `3` = setgid(2) + sticky(1). 나머지 `775` 는 소유자·그룹 rwx, 기타 r-x.
...
- `%G` — 소유 그룹 이름. `&&` 뒤 `ls -ld` 는 디렉터리 자체의 심볼릭 권한(`drwxrwsr-t`)을 보여 준다.
...
- `setfacl -m` — ACL 항목을 추가/수정(modify)한다. 형식 `u:<사용자>:<권한>` (그룹은 `g:`).
...
- `getfacl` — 파일의 소유자·그룹·ACL 항목 전체와 `mask`(ACL 최대 허용치)를 보여 준다.
...
- 디렉터리에 `x` 만 주면 목록 보기(r) 없이 **통과**만 허용한다. 경로 두 개(`~`, `~/work`)에 한 번에 적용.
...
- `sudo -u <사용자> <명령>` — 다른 사용자로 명령을 실행해 실제 권한을 검증한다.
...
- `sh -c '…'` — 리다이렉트(`>>`)까지 audit 권한으로 실행되도록 셸을 통째로 audit 으로 띄운다.
- `~learner` — learner 사용자의 홈 디렉터리(audit 입장에서 `~` 는 자기 홈이므로).
...
- `find /` — 루트부터 재귀 탐색. `-xdev` — 다른 파일시스템(/proc 등)으로 넘어가지 않는다.
- `-perm -4000` — SUID 비트가 **포함된** 파일(`-` 는 "이 비트들을 모두 가진"). `-type f` — 일반 파일만.
- `> ~/work/suid.txt` — 결과 목록을 파일로 저장.
...
- `ls -l` — 권한(`-rw-rw-rw-` = 666)과 소유자·그룹을 확인한다.
...
- `640` — 소유자 rw, 그룹 r, 기타 없음. 앱이 그룹 권한으로 읽을 수 있게 남기고 나머지는 막는다.
